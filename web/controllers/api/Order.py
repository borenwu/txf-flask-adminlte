...
def orderCreate():
    ...
    resp = target.createOrder(member_info.id,member_info.platform_id,member_info.community_id,member_info.community_name, items, params)
...
def orderCallback():
    ...
    if 'device_info' in callback_data:
        print("----------------has device info-----------------")
        stamp = callback_data['device_info']
        print(stamp)

        config_mina = app.config['MINA_APP'][stamp]
        print(config_mina)

        target_wechat.merchant_key = config_mina['paykey']
