...
def index():
    ...
    query = Food.query.filter(Food.platform_id == g.current_user.platform_id)
    if 'mix_kw' in req:
    ...
    if g.current_user.community_id is not None:
        return ops_render("food/leader.html")
...
def set():
    if request.method == "GET":
        ...
        communities = Community.query.filter(Community.platform_id == current_user.platform_id).filter_by(status=1)
        resp_data['info'] = info
    ...
    limit_quantity = int(req['limit_quantity']) if 'limit_quantity' in req and req['limit_quantity'] else 0
...
# @route_food.route("/test", methods=["POST"])
# def test():
#     print(request)
#     req = request.values
#     print(req)
#     stamp = req['stamp'] if ('stamp' in req and req['stamp']) else None
#     print(stamp)
#     resp = {'code': 200, 'msg': '操作成功~~', 'data': {}}
#     resp['data']['stamp'] = stamp
#     return jsonify(resp)